    ...
    <tbody>
        <tr>
            <td style="text-align: center; font-weight: normal; border-bottom: 1px solid black;">
                @if($registrant->student->phoneHome)
                    {{ $registrant->student->phoneHome }}
                @else
                    No Home Phone
                @endif
            </td>
            <td style="text-align: center; font-weight: normal;">
                @if($registrant->student->phoneMobile)
                    {{ $registrant->student->phoneMobile }}
                @else
                    No Cell Phone
                @endif
            </td>
            <td style="text-align: center; font-weight: normal;">
                @if($registrant->student->guardians->count() && $registrant->student->guardians->first()->user_id)
                    {{ $registrant->student->guardians->first()->phoneMobile }}
                @else
                    No Parent Phone
                @endif
            </td>
...
<table class="endorsements" >
    ...
    <tr>
        <td style="text-align: justify;">
            ...
            {{$registrant->student->gradeClassof}} at {{$schoolname}}.  We understand, in order to audition, that {{$registrantfirstname}}:
            <ol>
                <li>
...
<table class="detailDeclaration">
    ...
    <tbody>
        <tr>
            ...
            <td style="text-align: center; font-weight: normal;">
                @if($registrant->student->guardians->count() && $registrant->student->guardians->first())
                    {{ $registrant->student->guardians->first()->phoneMobile }}
                @else
                    No Parent Phone
                @endif
