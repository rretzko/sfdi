...
class FileviewportInternal extends Model
{
    ...
    public function __construct(\App\Registrant $registrant, \App\Filecontenttype $filecontenttype)
    {
    ...
    {
        //file exists
        if(\App\Fileupload::where('registrant_id', $this->registrant->id)
            ->where('filecontenttype_id', $this->filecontenttype->id)
            ->exists()){

            $fileupload = \App\Fileupload::where('registrant_id', $this->registrant->id)
                ->where('filecontenttype_id', $this->filecontenttype->id)
                ->first();
    ...
    private function mp3player(\App\Fileupload $fileupload)
    {
